...
public enum ImageResource {
    ...
    public static ImageResource getPlayer(Direction direction) throws IllegalArgumentException {
        return valueOf("PLAYER_" + direction);
    }

    public static ImageResource getMonster(Direction direction) throws IllegalArgumentException {
        return valueOf("MONSTER_" + direction);
    }

    public static ImageResource getBomb(int i) throws IllegalArgumentException {
        if (i < 0 || i > 3) throw new IllegalArgumentException("i must be between 0 and 3.");
        return valueOf("BOMB_" + i);
    }

    public static ImageResource getDigit(int i) throws IllegalArgumentException {
        if (i < 0 || i > 9) { throw new IllegalArgumentException("i must be between 0 and 9."); }
        return valueOf("DIGIT_" + i);
    }
